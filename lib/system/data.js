...
function init() {
    ...
    function getListSIP() {
        ...
        if (sip.connections && Object.keys(sip.connections).length) {
            for (var i in sip.connections) {
                var account = sip.connections[i].auth;
                data.push({ id: i, name: account.user + '@' + (account.domain || account.host) });
            }
        }
        return data;
    }
    ...
    bus.on('setB24accounts', function(accounts) {
        ...
        bus.emit('updateData', { source: 'b24accounts', b24accounts });
    });
    bus.onRequest('sipClients', function(param, cb) {
        var sipClients = [];
        if (config.get("sipServer") && config.get("sipServer")['sipClients']) {
            sipClients = config.get("sipServer")['sipClients'];
        }
        cb(null, sipClients);
    });
