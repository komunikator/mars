...
function init() {
    ...
    bus.on('RtpClose', function(data) {
		...
		sipAccount = sipAccounts[sipAccountID];
            ...
            if (sipAccount.availableTime && sipAccount.availableTime > 0)
		sipAccount.availableTime -= duration;

    	    bus.emit('updateData', {source:'config', data:{sipAccounts:sipAccounts}});
            bus.emit('refreshAvalibleTime', {sessionID:data.sessionID, sipAccountID:sipAccountID, sipAccount:sipAccount});
