...
//       sip.dialogs[data.sessionID].lastSttSeq += '' + text;
    ...
    bus.on('incomingCall', function (data) {
        ...
        bus.emit('startScript', {sessionID: data.sessionID, response: data.response, uri: data.response.headers.from.uri, script: data.script, sipAccountID: data.sipAccountID});
        // }
    });
    ...
    function startScript(params) {
        ...
        else
        {
            ...
            //console.log(sip.dialogs[sessionID]);
            //console.log(sipAccountID, sip.contact);
            if (!sip.parseUri(uri))
            {
            ...
            if (sip && sip.contact[sipAccountID] && scripts.length)
            {
                var scriptCfg = getScriptData(script);
                script = getScriptPath(script).replace('./scripts/', '');
                if (scriptCfg) {
                    bus.emit('scriptStatus', {category: 'call', sessionID: sessionID, type: 'info', msg: 'Start script [' + script + '] from [' + sipAccount + '] to uri [' + uri + ']'+(sip.dialogs[sessionID].params?' with params ['+sip.dialogs[sessionID].params.toString()+']':'')});
                    ...
                    bus.emit('runScript', {sessionID: sessionID});
                }
                else
