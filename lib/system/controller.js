...
function init() {
    ...
    bus.on('RtpClose', function (data) {
        if (sip.dialogs[data.sessionID]) {
            ...
            if (data && data.sessionID && sip.dialogs[data.sessionID] && sip.dialogs[data.sessionID].meta.status == "callFailed" && sip.dialogs[data.sessionID]._worker) sip.dialogs[data.sessionID]._worker.kill()
