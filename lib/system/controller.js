...
//       sip.dialogs[data.sessionID].lastSttSeq += '' + text;
    ...
    function startScript(params) {
        ...
        {
            var sessionID = params.sessionID || bus.newSessionID();
            ...
            if (sip && sip.contact[sipAccountID] && scripts.length)
            {
                ...
                if (scriptCfg) {
                    bus.emit('scriptStatus', {category: 'call', sessionID: sessionID, type: 'info', msg: 'Start script [' + script + '] from [' + sipAccount + '] to uri [' + uri + ']'});
