...
function init() {
                ...
                if (scriptCfg) {
                    ...
                    if (sip.dialogs[sessionID] && sip.dialogs[sessionID].response && sip.dialogs[sessionID].response.content){
                        if ( isWebRtcCall(sip.dialogs[sessionID].response.content) ) {
                            sip.dialogs[sessionID].webRtc = true;
                        } else {
    ...
    function requestData(req, params, cb) {
        console.log('controller.js requestData');
        req.params = params;

        bus.request('on_b24_request', req.params, (err, requestData) => {
            console.log('controller.js cb on_b24_request');
            delete req.params;
            cb(err, requestData);
        });
    }
