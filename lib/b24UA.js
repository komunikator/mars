function init() {
    ...
    function refreshB24accounts() {
        ...
        for (let key in connections) {
            if (!b24accounts || (b24accounts && !b24accounts[key]) ) {
                delete connections[key];
            }
        }
    ...
    bus.on('refresh', function(type) {
        ...
        for (let key in connections) {
            if (connections[key] && connections[key].auth && connections[key].auth.clientId && connections[key].auth.clientSecret && 
                connections[key].auth.disable != 1 && (!connections[key].auth.access_token && connections[key].auth.refresh_token)) {
    ...
    function registerB24botApi(task, nameTask, cb) {
        ...
        if (b24id && b24id.length) {
            ...
            connections[b24id] = connections[b24id] || {};
