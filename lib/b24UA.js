function init() {
    ...
    function refreshB24accounts() {
        ...
        for (let key in copyConnections) {
            ...
            delete copyConnections[key].chatBots;
        }

        bus.emit('setB24accounts', JSON.stringify(copyConnections));
        */
    };

    // ******************** обработка входящих сообщений от b24 по rest ******************** //
    function saveB24accounts() {
        console.log('saveB2accounts');
        let accounts = {};

        for(let key in connections) {
            console.log('key in connections: ', key);
            console.log('chatBots: ', connections[key].chatBots);
    ...
    bus.on('refresh', function(type) {
        ...
        for (let key in connections) {
            if (connections[key] && connections[key].auth && 
                connections[key].auth.refresh_token && !connections[key].auth.access_token &&
                connections[key].auth.clientId && connections[key].auth.clientSecret) {
                console.log('Нет access_token аккаунта: ', key);
                ...
                b24botApi.onRefreshTokens(req, (err, data) => {
                    ...
                    if (data && data.access_token && data.refresh_token) {
                        console.log('Обновление access_token, refresh_token аккаунта ', key);
    ...
    function registerBot(name, cb) {
        console.log('register bot name task: ', name);
        if ( fs.existsSync(`tasks/${name}`) ) {
            let task = require(`../tasks/${name}`);
            console.log('start register bot name task: ', name);
    ...
    function registerB24botApi(task, nameTask, cb) {
        ...
        if (b24id && connections[b24id] && connections[b24id].auth && connections[b24id].auth.access_token && connections[b24id].auth.portalLink) {
            ...
            b24botApi.onAppInstall(req, (err, data) => {
                ...
                console.log('chatBots: ', connections[b24id].chatBots);
                bus.emit('setB24accounts', JSON.stringify(connections));
