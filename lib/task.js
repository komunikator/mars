                            ...
                            var job = new cronJob(
                                    {
                                        ...
                                        onTick: function () {
                                            bus.request('targetData', {name: c.target.replace(/\.js$/, '')}, function (err, data) {
                                                if (data.items && data.items.length) {
                                                    bus.emit('message', {type: 'debug', msg: ' Job from [' + c.name + '] onTime: "' + c.startTime + '" added'});
                                                    data.items.forEach(function (params) {
                                                        bus.emit('startScript', {uri: params.shift(), script: c.script, sipAccountID: c.sipAccountID, params: params});
                                                    });
                                                }
                                                else
