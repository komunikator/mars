                ...
                Tasks.forEach(function(c) {
                    if (c.active == 'true') {
                        ...
                        if (c.onEvent) {
                            ...
                            bus.emit('setOnEventTask', c.name);
                                    ...
                                    bus.request('targetData', { query: { name: c.target.replace(/\.js$/, '') } }, function(err, data) {
                                        if (data.items && data.items.length) {
                                            ...
                                            data.items.forEach(function(params) {
                                                bus.emit('startScript', { uri: params.shift(), script: c.script, sipAccountID: c.sipAccountID, params: params, parentID: parentID, rejectTime: rejectTime, callsCount: parseInt(c.callsCount) });
                                            });
                                        } else
