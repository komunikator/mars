                ...
                Tasks.forEach(function (c) {
                    if (c.active == 'true') {
                        ...
                        if (c.onEvent)
                        {
                            ...
                            bus.emit('setOnEvent', {event: c.onEvent, script: c.script});
                        }
                        else
                        if (c.startTime) {
                                            ...
                                            bus.request('targetData', {query:{name: c.target.replace(/\.js$/, '')}}, function (err, data) {
                                                if (data.items && data.items.length) {
                                                    ...
                                                    data.items.forEach(function (params) {
                                                        bus.emit('startScript', {uri: params.shift(), script: c.script, sipAccountID: c.sipAccountID, params: params, parentID: parentID, rejectTime: rejectTime});
                                                    });
                                                }
                                                else
                            ...
                            // var rejectMin = parseInt(c.rejectTime.split(":")[1]); 
                            // var rejectHour = parseInt(c.rejectTime.split(":")[0]); 
                            // var cron_rejectTime = rejectMin + ' ' + rejectHour + ' * * *';
                            // var stop_job = new cronJob({
                            //     cronTime: '00 ' + cron_rejectTime,
                            //     onTick: function() {
                            //         bus.request('targetData', {query:{name: c.target.replace(/\.js$/, '')}}, function (err, data) {
                            //             bus.emit('message', {type: 'debug', msg: ' Job from [' + c.name + '] onTime: "' + c.rejectTime + '" stopped'});
                            //         });
                            //     },
                            //     start: true,
                            //     context: c
