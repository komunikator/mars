...
function init() {
    ...
    worker.on('message', function (d) {
        if (d.action === undefined) {
            bus.emit('message', { category: 'call', sessionID: sessionID, type: 'error', msg: 'RTP ' + JSON.stringify(d) });
            return;
        }
        ...
        switch (d.action) {
            ...
            case 'sttInit':
                ...
                break;
            case 'sttOn':
                bus.emit('message', { category: 'call', sessionID: sessionID, type: 'debug', msg: 'sttOn' + JSON.stringify(d) });
                // bus.emit('message', { category: 'call', sessionID: sessionID, type: 'debug', msg: 'sttOn' + JSON.stringify(d.params.text) + sessionID });
            ...
            case 'direction':
                ...
                break;
            default:
                bus.emit('message', { category: 'call', sessionID: sessionID, type: 'debug', msg: 'RTP ' + JSON.stringify(d) });
                break;
        }
    });
