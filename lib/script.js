...
function onAnswered() {
    ...
    function setParams(obj) {
        ...
        for (var key in n_obj) {
            ...
            {
                n_obj[key] = n_obj[key].apply( getScriptParams() );
            }
        ...
        function getAction(obj, k) {
            ...
            switch (k) {
                ...
                case 'dtmfData':
                    f = function () {
                        bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Script set '" + k + "' state"});// + JSON.stringify(obj[k])});
                        bus.emit('dtmfData', {sessionID: sessionID, dtmfDataMode: obj[k], seq: dtmfKeys || sttText});
                        if (cb)
                            cb();
                    };
                    break;
                case 'recOn':
                    f = function () {
                        bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Script set '" + k + "' state"});// + JSON.stringify(obj[k])});
                        var o = setParams({rec: obj[k], file: obj[k].file || recPath});
                        session.rec(o);
                        if (cb)
                            cb();
                    };
                ...
                case 'action':
                        ...
                        scriptParams.cb = function(res){ actionRes = res; if (cb)  cb()};
                        if (typeof obj[k].func == 'function' || obj[k].func instanceof Function)
                        {
                            var func = obj[k].func || function(){ this.cb() };
