            ...
            switch (k) {
                ...
                case 'sendDtmf':
                    f = function () {
                        ...
                        var interval = 1000;
                        if (o.interval >= 0) interval = o.interval;
                        ...
                        if ( (!isNotCorrect) && (o.text) ) {
                            arrDtmf = o.text.match(/[A-D0-9R*#]/gi);
                            //console.log(arrDtmf);
                        } else {
                            bus.emit('message', {category: 'call', type: 'error', msg: "Dtmf is not correct " + o.text});
                        }
			   ...
			   if (session.sttOpt.model)
                        ...
                        if (obj[k]) {
                            ...
                            session.sttEvent = function (data) {
                                if (data.seq != undefined) {
                                    sttText = data.seq;
                                    if (!workCompleted)
                                        onStt(obj[k].keys || obj[k], data.seq);
                                }
                            };
                        }
                        ;
                ...
                case 'dtmfOn':
                    f = function () {
                        ...
                        bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Script set '" + k + "' state" + (!session.dtmfOpt ? "" : (". Option:" + JSON.stringify(session.dtmfOpt)))});// + JSON.stringify(obj[k])});
                        var workCompleted;
                        ...
                        if (obj[k]) {
                            ...
                            session.onDtmf = function (data) {
                                if (data.seq !== undefined) {
                                    dtmfKeys = data.seq;
                                    if (!workCompleted)
                                        onDtmf(obj[k].keys || obj[k], data.seq);
                                }
                            };
                        }
                        ;
