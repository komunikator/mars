...
function makeResponse(rq, status, reason) {
    return {
        ...
        version: rq.version,
        headers: {
            'record-route': rq.headers['record-route'] || [], //  *** DIGT Telecom ***
            via: rq.headers.via,
            to: rq.headers.to,
    ...
    return {
        open: function (remote, error) {
            return {
                send: function (m) {
                    var s = stringify(m);
                    socket.send(new Buffer(s, 'utf8'), 0, Buffer.byteLength(s, 'utf8'), remote.port, remote.address);
                },
                protocol: 'UDP',
                release: function () {
                }
            ...
            send: {
                value: function (m) {
                    if (m.method) {
                        ...
                        m.headers.via[0].port = Number(options.port) || defaultPort(this.protocol);
...
function resolve(uri, action) {
    ...
    if (!uri.port && uri.host == 'multifon.ru') {
        uri.port = 5060;
    }
...
function createInviteServerTransaction(transport, cleanup) {
    ...
    return {
        send: sm.signal.bind(sm, 'send'), message: sm.signal.bind(sm, 'message'), shutdown: function () {
            sm.enter(terminated);
        }
    };
}
...
function createInviteClientTransaction(rq, transport, tu, cleanup, options) {
    ...
    return {
        message: sm.signal.bind(sm, 'message'), shutdown: function () {
            sm.enter(terminated);
        }
    };
}
...
function getNextHop(rq) {
    var hop = {};
    if (rq.headers['record-route'] || rq.headers['route']) {
        const routeUri = rq.headers['record-route'] ? rq.headers['record-route'][0].uri : rq.headers['route'][0].uri;
        hop = parseUri(routeUri)
    } else hop = parseUri(rq.uri);
    if (!hop && rq.headers.route) {
...
function makeTransactionLayer(options, transport) {
    ...
    return {
        createServerTransaction: function (rq, remote) {
            ...
            var cn = transport(remote, function () {
            }, true);
                ...
                function next() {
                    ...
                    if (address.length > 0) {
                        try {
                            ...
                            var cn = transport(address.shift(), function (e) {
                                client_transactions[id].message(makeResponse(rq, 503));
                            });
