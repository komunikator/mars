            ...
            if (rs.headers.to.params.tag) { // check if it's an in dialog request
                if (sessionID && dialogs[sessionID])
                {
                    switch (rs.method)
                    {
                        ...
                        case 'NOTIFY':
                            if (rs.headers.event && 
                                rs.headers.event.toLowerCase() === 'refer' &&
                                rs.headers['subscription-state'] && 
                                /^terminated/i.test(rs.headers['subscription-state'])){
...
//                        if (sip.status[sipAccountID] !== 'registered' && sip.status[sipAccountID] !== 'registration') {
    ...
    sip.refer = function (sessionID, target) {
        ...
        var cSequence = Math.floor(Math.random() * 1e5);
        var i=0;
        var account;
        var rq = sip.dialogs[sessionID].sipContext;
        rq.method = 'REFER';
        ...
        rq.headers['Refer-To'] = '<sip:' + account + '@' + sip.parseUri(rq.headers.from.uri).host + '>';
        rq.headers['Referred-By'] = '<' + rq.headers.to.uri + '>';

        if (Array.isArray(target)){ 
            bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Transfer accepted to '" + target[i] + "'"});
            sip.sendRq(rq, sendRqCallback, sessionID);
        } else {
            rq.headers['Refer-To'] = '<sip:' + account + '@' + sip.parseUri(rq.headers.from.uri).host + '>';//target;
            sip.sendRq(rq, function (rs) {
        ...
        function sendRqCallback (rs) {
            bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "===================" + rs.status + "==================="});
            delete rq.headers['Refer-To'];
            delete rq.headers['Referred-By'];
