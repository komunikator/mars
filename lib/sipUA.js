...
function init() {
    ...
    function register(cb, sipAccountID, unregister) {
        ...
        try {
            var rq = {
                method: 'REGISTER',
                uri: 'sip:' + sip.connections[sipAccountID].auth.user + ':' + sip.connections[sipAccountID].auth.password + '@' + sip.connections[sipAccountID].auth.host + ';transport=' + sip.connections[sipAccountID].auth.transport,
                headers: {
        ...
        if (sip.connections && Object.keys(sip.connections).length) {
            for (var key in sip.connections) {
                    ...
                    if (!sip.connections[key].status || sip.connections[key].status == 'disabled') {
                        ...
                        // sip.options('sip:1:1@' + sip.connections[key].auth.host, key, function optionsResult(sipAccountID) {
