...
function init() {
    ...
    function register(cb, sipAccountID, unregister) {
        ...
        var expiresTime = (unregister == undefined ? sip.connections[sipAccountID].contact.params.expires : 0);
        var rq = {
            method: 'REGISTER',
    ...
    function registerAccountEvent() {
        ...
        function isChangeSipAccounts() {
            if (!sip.lastAuth) return true;
            var sipAccountsData = JSON.stringify(config.get("sipAccounts")),
    ...
    function restartSipConnections() {
        ...
        if (!Object.keys(sip.connections).length) {
            ...
            var sipAccounts = config.get("sipAccounts");
            for (var key in sip.connections) {
                if (sip.connections[key].status === 'registered' && (!sip.connections[key].auth || (sip.connections[key].auth && sip.connections[key].auth.disable))) {
                    countContacts++;
