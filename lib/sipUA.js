...
//detect SIP public ip
                    ...
                    if (rs.headers.via[0].params &&
                        ...
                        bus.emit('message', {type: 'info', msg: 'SIP UA' + sipAccountID + ' [' + sip.from[sipAccountID].uri + '] detected public ip: "' + host + ':' + port + '"'});
                        //consolbus.log('Public ip:port ' + host + ':' + port);
                        ...
                        //consolbus.log(rq.headers.contact && rq.headers.contact[0].uri);
                    }
//
                            ...
                            //sip.send(rq,
                                    function (rs_) {
                                        ...
                                        else
                                        {
                                            ...
                                            bus.emit('message', {type: 'debug', category: logCategory, msg: 'SIP method "' + rq.method + '" authorization failed with status ' + rs_.status + '. Reason: "' + rs_.reason + '"'});
                                        }
                                    }
                            );
                        }
            ...
            if (rs.headers.to.params.tag) { // check if it's an in dialog request
                ...
                if (sessionID && dialogs[sessionID])
                {
                    switch (rs.method)
                    {
                        ...
                        case 'CANCEL':
                        case 'BYE':
                            bus.emit('callEnded', {uri: rs.headers.from.uri, sessionID: sessionID});
                            var rs_ = sip.makeResponse(rs, 200, 'OK');
                            sip.setHeaders(rs_, sipAccountID);
                            sUA[sipAccountID].send(rs_);
                            break;
                ...
                if (rs.method === 'INVITE') {
                    ...
                    bus.request('onEvent', {id: 'on_call[' + sipAccountID + ']'}, function (err, data) {
                        ...
                        if (data) {
                            ...
                            bus.emit('ringing', {sessionID: sessionID, uri: rs.headers.from.uri});
                            setTimeout(function () {
                                bus.emit('incomingCall', {response: rs, sessionID: sessionID, script: script, sipAccountID: sipAccountID});
                            }, 1000);
                        }
                        else {
...
//                'content-length': 0
    ...
    sip.startUA = function () {
        ...
        var toDo = function () {
            if (sip.auth[sipAccountID])
                new findPort(config.get("sipPort")).getPort(function (port) {
                    //console.log(sip.auth[sipAccountID].disable);
    ...
    sip.bye = function (sessionID) {
        ...
        sip.sendRq(rq, function () {
            bus.emit('callEnded', {sessionID: sessionID, uri: rq.headers.to.uri});
        }, sessionID);
    };
    ...
    sip.cancel = function (sessionID, reason) {
        ...
        rq.method = 'CANCEL';
        rq.headers.cseq = {method: rq.method, seq: 1};
