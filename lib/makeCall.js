...
//e.emit('start_call', call_id);
    ...
    this.sendMESSAGE = function (sipAccountID, text, to, cb_func) {
        if (!dialogs[sessionID] || !dialogs[sessionID].sipContext)
        {
            e.emit('message', {category: 'call', sessionID: sessionID, type: 'error', msg: "Dialog not found"});
            //e.emit('callEnded', {sessionID: sessionID});
        ...
        // если передан адрес для отправки сообщения, используем его, иначе используем адрес отправителя
        // to - адрес, куда отправлять сообщение
        var to_uri = to ? to : dialogs[sessionID].sipContext.headers.to.uri;
