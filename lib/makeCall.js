var bus = require('./system/bus'),
        config = bus.config,
        sip = require('./sip/sip'),
        sdp = require('./sip/sdp'),
        findPort = require('./findPort.js');

var rtpContent = function (data_obj) {
    if (!data_obj.port)
        data_obj = {port: data_obj};
    var data =
            'v=0\r\n' +
            'o=- 13374 13374 IN IP4 ' + (data_obj.host || sip.hostIp) + '\r\n' +
            's=-\r\n' +
            'c=IN IP4 ' + (data_obj.host || sip.hostIp) + '\r\n' +
            't=0 0\r\n' +
            'm=audio ' + data_obj.port + ' RTP/AVP 0 101\r\n' +
            //'m=audio ' + data_port + ' RTP/AVP 0 8 101\r\n' +
            'a=rtpmap:0 PCMU/8000\r\n' +
            //'a=rtpmap:8 PCMA/8000\r\n' +
            'a=rtpmap:101 telephone-event/8000\r\n' +
            'a=fmtp:101 0-15\r\n' +
            'a=ptime:30\r\n' +
            'a=sendrecv\r\n';
    return data;
};

// Making the call
function call(sessionID, rs, data_port) {
    var session = sip.dialogs[sessionID],
            sipAccountID = session.sipAccountID,
            dtmf_payload_type = 101; //def


    bus.emit('message', {category: 'debug', sessionID: sessionID, msg: 'Start dialog with call-id "' + rs.headers['call-id'] + '"'});
    //bus.emit('message',{type:'info', msg: 'Set RTP_IN port: ' + data_port);

    // yes we can get multiple 2xx response with different tags
    if (!rs.content)
        return;
    var sdp_discr = sdp.parse(rs.content).m[0];
    if (sdp_discr && sdp_discr['a'])
        for (var n in sdp_discr.a)
        {
            var match = /rtpmap:(\d+) telephone-event/i.exec(sdp_discr.a[n]);
            if (match)
                dtmf_payload_type = match[1];
        }
//       if (dtmf_payload_type !== undefined)
//           bus.emit('message',{type:'info', msg: "Detected DTMF Payload Type " + dtmf_payload_type);

    if (!session.response)//only for outgoing call
    {//sending ACK
        var rq = {
            method: 'ACK',
            uri: rs.headers.contact[0].uri,
            headers: {
                to: rs.headers.to,
                from: rs.headers.from,
                'call-id': rs.headers['call-id'],
                cseq: {method: 'ACK', seq: rs.headers.cseq.seq},
                via: [],
                //via: [rs.headers.via[0]],
                'max-forwards': 71
            }
        };
        sip.setHeaders(rq, sipAccountID);
        //sip.send(rq);
        sip.sUA[sipAccountID].send(rq);
    }
    var rtp_ip = (sdp.parse(rs.content).c && sdp.parse(rs.content).c.address) || (sdp.parse(rs.content).m && sdp.parse(rs.content).m[0] && sdp.parse(rs.content).m[0].c && sdp.parse(rs.content).m[0].c.address);
    var rtp_port = sdp_discr.port;

    bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Parsing SDP: RTP_OUT " + rtp_ip + ":" + rtp_port});

    if (dtmf_payload_type !== undefined)
        bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Parsing SDP: DTMF Payload Type " + dtmf_payload_type});

    var info = {
        from: sip.parseUri(rs.headers.from.uri).user,
        to: sip.parseUri(rs.headers.to.uri).user,
        out: {
            ip: rtp_ip,
            port: rtp_port},
        in: {
            ip: sip.hostIp,
            port: data_port,
            dtmf_payload_type: dtmf_payload_type},
        sessionID: sessionID
    };

    //rtp = new rtpWorker(e, info, sip.dialogs);
    bus.emit('rtpWorker', info);

    // registring our 'dialog' which is just function to process in-dialog requests
    // refer from name
    var m = rs.headers.from.name;
    if (m)
        m = m.match(/referred-by\<(.*)\>/);
    if (m)
        info.refer = m[1];
    //reref from sip
    if (rs.headers['referred-by'])
        info.refer = sip.parseUri(rs.headers['referred-by'].replace(/^\<(.*)\>/, '$1')).user;
    info.type = session.response ? 'incoming' : 'outgoing';
    info.status = 'answered';
    info.script = sip.dialogs[sessionID].meta.script;
    info.pin = sip.dialogs[sessionID].meta.pin;
    info.times = sip.dialogs[sessionID].meta.times;
    info.times.answered = new Date().getTime();
    // для входящего звонка в объекте sip.dialogs в заголовке переставляем значения полей from и to
    sip.dialogs[sessionID].dialogID = sip.getDialogID(rs);
    sip.dialogs[sessionID].sipContext = {
        uri: rs.headers.contact[0].uri,
        headers: {
            from: rs.headers.to,
            to: rs.headers.from,
            'call-id': rs.headers['call-id']
        }
    };
    // для корректного завершения звонка подставляем в поле route, если оно есть, значение поля record-route
    if (rs.headers['record-route'])
        sip.dialogs[sessionID].sipContext.headers['route'] = rs.headers['record-route'];

    sip.dialogs[sessionID].meta = info;
    // если звонок исходящий в объекте sip.dialogs в заголовке значения полей from и to переставляем обратно
    if (info.type === 'outgoing')
    {
        sip.dialogs[sessionID].sipContext.headers.from = rs.headers.from;
        sip.dialogs[sessionID].sipContext.headers.to = rs.headers.to;
    }

    //console.log(sip.dialogs);
    bus.emit('answered', {sessionID: sessionID, uri: sip.dialogs[sessionID].sipContext.headers.to.uri});
}

function make_call(sessionID, sipAccountID, data_port) {
    var session = sip.dialogs[sessionID],
            uri = session.uri;

    var ringingTimeout = config.get("ringingTimeout") || 60, //sec
            stunServer = config.get("stunServer"),
            ringingTimeoutID;

    if (stunServer) {
        stunServer = stunServer.split(':');
        stunServer = {host: stunServer[0], port: stunServer[1] || 3478}
    }

    var content;
    if (sip.sUA[sipAccountID].publicIP && stunServer) {
//RTP Stun
        var stun = require('vs-stun');
        var socket = require('dgram').createSocket('udp4');
        //TODO Move to rtp.js
        socket.on('listening', function ( ) {
            bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: 'STUN request to STUN server ' + stunServer.host + ':' + stunServer.port});
            stun.resolve(socket, stunServer, function (err, value) {
                socket.close();
                if (value && value.public) {
                    bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: 'Resolved RTP external IP:port (' + value.public.host + ':' + value.public.port + ')'});
                    content = rtpContent(value.public);
                    toDo();
                }
                else
                {
                    bus.emit('message', {category: 'call', sessionID: sessionID, type: 'error', msg: 'STUN request to STUN server ' + stunServer.host + ':' + stunServer.port + ' has timed out. ' + value.type});
                    //???? maybe re-INVITE
                    content = rtpContent({host: sip.sUA[sipAccountID].publicIP, port: data_port});
                    toDo();
                }
            }, {count: 1, timeout: 100});
        });

        socket.bind(data_port);
    }
    else
    {
        content = rtpContent(data_port);
        toDo();
    }
    function toDo() {
        if (session.response) {
            // bus.emit('message', {category: 'call', sessionID:sessionID, type: 'info', msg: 'URI [' + uri + '] is is called'});
            /*
             session.response.headers.to.params.tag = sip.newTag();
             var rs = sip.makeResponse(session.response, 180, 'Ringing');
             sip.setHeaders(rs);
             sip.send(rs);
             */

            setTimeout(function () {
                if (!session.inviteAckCompleted) {
                    bus.emit('callCancelled', {msg: "Invite ACK timeout", sessionID: sessionID, uri: session.response.headers.to.uri});
                }
            }, 10000); //wait ACK timeOut

            session.meta.in = {
                ip: sip.hostIp,
                port: data_port
            };

            if (sip.dialogs[sessionID]) {
                var rs = sip.makeResponse(session.response, 200, 'OK');
                rs.headers['content-type'] = 'application/sdp';
                rs.content = content;
                sip.setHeaders(rs, sipAccountID);
                //sip.send(rs);
                sip.sUA[sipAccountID].send(rs);
            }
        }
        else
        {   //TODO move to sipUA
            var rq = {
                method: 'INVITE',
                uri: uri,
                headers: {
                    to: {uri: uri},
                    from: {name: sip.from[sipAccountID].name, uri: sip.from[sipAccountID].uri, params: {tag: sip.newTag()}},
                    'call-id': sip.newCallId(sip),
                    cseq: {method: 'INVITE', seq: 1},
                    'content-type': 'application/sdp',
                    contact: [sip.contact[sipAccountID]]
                },
                content: content
            };
            bus.emit('message', {category: 'call', sessionID: sessionID, type: 'info', msg: 'Call to [' + uri + ']'});
            var data = {
                sessionID: sessionID,
                meta: {
                    type: 'outgoing',
                    status: 'invite',
                    from: sip.parseUri(rq.headers.from.uri).user,
                    to: sip.parseUri(rq.headers.to.uri).user,
                    in: {
                        ip: sip.hostIp,
                        port: data_port
                    }
                }};
            bus.emit('invite', data);
            //console.log('invite', data.sessionID);
            if (!sip.dialogs[sessionID])
                sip.dialogs[sessionID] = {meta: data.meta};

            sip.dialogs[sessionID].inviteRq = rq;//TODO 

            ringingTimeoutID = setTimeout(
                    function () {
                        if (sip.dialogs[sessionID].meta.status === 'ringing') //just to be safe
                            sip.cancel(sessionID, 'Service cancel INVITE. Reason: ringing timeout [' + ringingTimeout + ' sec]');
                    },
                    ringingTimeout * 1000);
            var onceFlag = true;
            sip.sendRq(rq,
                    function (rs) {
                        if (onceFlag)
                        {
                            onceFlag = false;
                            if (!ringingTimeoutID)
                                return;
                            else
                                clearTimeout(ringingTimeoutID);

                            //console.log(rs);
                            if (rs.status === 200)
                            {
                                bus.emit('message', {category: 'call', sessionID: sessionID, type: 'info', msg: 'URI [' + uri + '] is answered'});
                                call(sessionID, rs, data_port);
                            }
                            else
                            if (rs.status !== 487)// !CANCEL
                            {
                                // bus.emit('message', {type: 'info', msg: 'INVITE failed with status ' + rs.status + '. Reason: "' + rs.reason + '"'});
                                bus.emit('callFailed', {sessionID: sessionID, uri: uri, type: 'info', msg: 'INVITE failed with status ' + rs.status + '. Reason: "' + rs.reason + '"'});
                            }
                        }
                        ;
                    }, sessionID, sipAccountID);
        }
//bus.emit('start_call', call_id);
    }
}
;
function getBusyPorts() {
    var ports = [];
    if (sip.port)
        ports = sip.port;
    if (sip.dialogs) {
        for (var i in sip.dialogs)
            if (sip.dialogs[i].meta.in)
                ports.push(sip.dialogs[i].meta.in.port);
    }
    return ports;
}


function init() {
    var sessionID = this.sessionID,
            session = sip.dialogs[sessionID],
            sipAccountID = session.sipAccountID;
    //console.log(session);
    new findPort().getPort(function (port) {
        bus.emit('message', {category: 'call', sessionID: sessionID, type: 'debug', msg: "Set RTP_IN port to " + port});
        make_call(sessionID, sipAccountID, port);
    }, getBusyPorts());
}


bus.on('inviteACK', function (data) {

    if (data.sessionID &&
            sip.dialogs &&
            sip.dialogs[data.sessionID] &&
            sip.dialogs[data.sessionID] &&
            sip.dialogs[data.sessionID].meta.status === 'start')
    {
        var session = sip.dialogs[data.sessionID];
        session.inviteAckCompleted = true;
        call(data.sessionID, session.response, session.meta.in.port);
    }
    //console.log('inviteACK', sip.dialogs[data.sessionID].meta);
});

bus.on('callEnded', function (data) {
    if (data.sessionID &&
            sip.dialogs &&
            sip.dialogs[data.sessionID] &&
            sip.dialogs[data.sessionID].close)
        sip.dialogs[data.sessionID].close(data);
});

bus.on('dtmf_seq', function (data) {
    if (data.sessionID &&
            sip.dialogs &&
            sip.dialogs[data.sessionID] &&
            sip.dialogs[data.sessionID].onDtmf)
        sip.dialogs[data.sessionID].onDtmf(data);
});

bus.on('stt_seq', function (data) {
    if (data.sessionID &&
            sip.dialogs &&
            sip.dialogs[data.sessionID] &&
            sip.dialogs[data.sessionID].sttEvent)
        sip.dialogs[data.sessionID].sttEvent(data);
});

bus.on('runScript', function (cntx) {
    init.apply(cntx);
});

module.exports = init;