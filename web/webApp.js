...
/*
 ...
 var result = ((username === 'admin') && (cipher.final('base64') === 'admin'));
...
exports.start = function(options, callback) {
    ...
    Events.on('getData', function(obj) {
        if (obj.source) {
            switch (obj.source) {
                case 'lang':
                    try {
                    ...
                    if (langSource)
                        obj.data = langSource.msg;
                    else
                        obj.data = {};
                    break;
            }
            if (obj.data !== undefined)
                Events.emit(obj.requestID, obj);
        }
    });
    ...
    Events.getData({source: 'webAccounts'}, function(obj) {
        users = obj.data || {};
    });


    router.init(app, models);
    var port = process.env.PORT || options.config.get("webPort") || 8080;
    try {
        //app.listen(port);
...
//https
        ...
        Events.emit('message', {type: 'info', msg: 'Web server start on port ' + port});
    }
    catch (e) {
        console.log(e);
    }
    ...
    webSocketServer.on('connection', function(ws) {
        ...
        Events.emit('message', {type: 'info', msg: 'Web User connected. Total web connections: ' + webSocketServer.clients.length});
//        Events.emit('updateData', {source: 'statusUA', data: []});

        ws.on('message', function(message) {
        });

        ws.on('close', function() {
            Events.emit('message', {type: 'info', msg: 'Web User disconnected. Total web connections: ' + webSocketServer.clients.length});
        });
    });
