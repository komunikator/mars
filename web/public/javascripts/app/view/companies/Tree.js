Ext.define('IVR.view.companies.Tree', {
    ...
    constructor: function(config) {
        ...
        self.nameTest = function(text, cfg, flag) {
            ...
            if (!(/^[ a-яА-Яa-zA-Z0-9_/-]+$/.test(text) && flag)) {
                var newMsg = '<span style="color:red">' + lang.enterCompanyName + ':</span>';
                cfg.value = text;
                    ...
                    items:
                            [
                                ...
                                },
                                {
                                    ...
                                    align: 'right',
                                    handler: function() {
                                        Ext.MessageBox.prompt(lang.createCompany, lang.enterCompanyName + ':', function(btn, text, cfg) {
                                            if (btn == 'ok') {
                                                if (!self.nameTest(text, cfg))
                                                    return;
                                                Ext.Ajax.request({
                                                    url: '/companyData/create',
                                                    method: 'post',
                                                    params: {
                                                        name: text
                                                    },
