...
// 1. Echo sockjs server
    ...
    sockjs_server.on('connection', function (conn) {
        ...
        conn.on('data', function (message) {
            ...
            function (err, data) {
                if (data && data.items)
                    data.items.forEach(function (item_) {
                    ...
                    if (data)
                        for (var key in data)
                            if (data[key] && data[key].meta && data[key].meta.pin == message) {
                                filter = key;
                                data[key].log && data[key].log.forEach(function (str) {
                                    format(str);
                                });
                            }
                });

                bus.on('message', onData);
            });
        });
        ...
        conn.on('close', function (message) {
            ...
            bus.removeListener('message', onData);
        });
    });
